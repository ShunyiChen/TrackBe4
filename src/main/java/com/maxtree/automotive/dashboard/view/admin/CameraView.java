...
public class CameraView extends ContentView {

    public CameraView(String parentTitle, AdminMainView rootView) {
...
//        loggedInUser = ui.userService.getUserByUserName(username);
        ...
        GridColumn[] columns = {new GridColumn("高拍仪",100),new GridColumn("模板文件",380), new GridColumn("已启用",80),new GridColumn("", 20)};
...
//                    }
            ...
            menu.addItem("启用", new Menu.Command() {
                @Override
                public void menuSelected(MenuItem menuItem) {
                    updateUsageState(camera);
                }
            });
            menu.addSeparator();
...
//                    }
        ...
        return new Object[] {camera.getName(),camera.getTemplatePath(),camera.getEnable()?"√":"", img, camera.getId()};
    }
     ...
     * @param cam
     */
    private void updateUsageState(Camera cam) {
        Iterable<Camera> iter = TB4Application.getInstance().cameraRepository.findAll();
        iter.forEach(c -> {
            if(cam.getId() == c.getId()) {
                c.setEnable(true);
            } else {
                c.setEnable(false);
            }
