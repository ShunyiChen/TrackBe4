...
public class EditCameraWindow extends Window {
    ...
    public EditCameraWindow() {
        ...
        form.addComponents(nameField, descField, inUseBox);
    ...
    private void bindFields() {
        ...
        binder.bind(descField, Camera::getDescription, Camera::setDescription);
    }
    ...
    private void validatingFieldValues () {
        ...
        binder.forField(descField).withValidator(new StringLengthValidator(
                "描述长度范围在0~40个字符",
                0, 40)) .bind(Camera::getDescription, Camera::setDescription);
    }
    ...
    public static void open(Callback2 callback) {
        ...
        w.btnAdd.addClickListener(e -> {
            if (w.checkEmptyValues()) {
                w.camera.setDescription(w.descField.getValue()==null?"":w.descField.getValue());
                w.camera.setInUse(w.inUseBox.getValue());
    ...
    public static void edit(Camera camera, Callback callback) {
        ...
        if (opt.isPresent()) {
            ...
            w.descField.setValue(c.getDescription()==null?"":c.getDescription());
            w.inUseBox.setValue(c.getInUse()==null?false:c.getInUse());
    ...
    private TextField descField = new TextField("描述:");
    private CheckBox inUseBox = new CheckBox("使用");
