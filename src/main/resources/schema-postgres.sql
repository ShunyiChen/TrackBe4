...
BEGIN
	...
	BATCH INT NOT NULL,--批次号,默认最大1000个批次，每批次最多放5000文件夹。
	...
	DETAILS VARCHAR(200),--描述json
	USERNAME VARCHAR(20) NOT NULL,--操作者用户名
...
LANGUAGE plpgsql$$
--获取分页数
DROP FUNCTION IF EXISTS CREATE_PAGINGCOUNT(LIMIT2 INT);
CREATE OR REPLACE FUNCTION CREATE_PAGINGCOUNT(LIMIT2 INT) RETURNS SETOF INTEGER AS
$BODY$
DECLARE
  A INTEGER := 0;
  B INTEGER := 0;
...
LANGUAGE PLPGSQL$$
--分页页数
DROP FUNCTION IF EXISTS CREATE_PAGINGCOUNT2(LIMIT2 INT,KEYWORD VARCHAR);
CREATE OR REPLACE FUNCTION CREATE_PAGINGCOUNT2(LIMIT2 INT,KEYWORD VARCHAR) RETURNS BIGINT AS
$BODY$
DECLARE
  B INTEGER := 0;
  L INTEGER := 0;
...
BEGIN
   ...
   IF L = 9 OR L =10 THEN
	FOR I IN 0..255 LOOP
	   EX_SQL := 'SELECT COUNT(TRANSACTIONUNIQUEID) FROM TRANSACTION_'||I||' WHERE PLATENUMBER ILIKE '''||KEYWORD||'''';
   ...
   ELSIF L = 15 THEN
	FOR I IN 0..255 LOOP
	   EX_SQL := 'SELECT COUNT(TRANSACTIONUNIQUEID) FROM TRANSACTION_'||I||' WHERE BARCODE ILIKE '''||KEYWORD||'''';
	EXECUTE EX_SQL INTO RS;
	   TOTAL := TOTAL + RS;
	END LOOP;
   ...
   ELSIF L = 19 THEN
	FOR I IN 0..255 LOOP
	   EX_SQL := 'SELECT COUNT(TRANSACTIONUNIQUEID) FROM TRANSACTION_'||I||' WHERE VIN ILIKE '''||KEYWORD||'''';
	EXECUTE EX_SQL INTO RS;
	   TOTAL := TOTAL + RS;
	END LOOP;
   ELSE
	FOR I IN 0..255 LOOP
	   EX_SQL := 'SELECT COUNT(TRANSACTIONUNIQUEID) FROM TRANSACTION_'||I||' WHERE VIN ILIKE '''||KEYWORD||''' OR BARCODE ILIKE '''||KEYWORD||''' OR PLATENUMBER ILIKE '''||KEYWORD||''' ';
	EXECUTE EX_SQL INTO RS;
	   TOTAL := TOTAL + RS;
	END LOOP;
...
--获取模糊查询
DROP FUNCTION IF EXISTS CREATE_SEARCH(LIMIT2 INT,OFFSET2 INT,KEYWORD VARCHAR);
CREATE OR REPLACE FUNCTION CREATE_SEARCH(LIMIT2 INT,OFFSET2 INT,KEYWORD VARCHAR) RETURNS SETOF TRANSACTION_0 AS
$BODY$
DECLARE
  L INTEGER := 0;
  EX_SQL VARCHAR;
BEGIN
   ...
   IF L = 9 OR L =10 THEN
	FOR I IN 0..254 LOOP
		EX_SQL := EX_SQL || 'SELECT * FROM TRANSACTION_'||I||' WHERE PLATENUMBER ILIKE '''||KEYWORD||''' UNION ALL ';
	END LOOP;
		EX_SQL := EX_SQL || 'SELECT * FROM TRANSACTION_255 WHERE PLATENUMBER ILIKE '''||KEYWORD||''' LIMIT '''||LIMIT2||''' OFFSET '''||OFFSET2||'''';
   ELSIF L = 15 THEN
	FOR I IN 0..254 LOOP
		EX_SQL := EX_SQL || 'SELECT * FROM TRANSACTION_'||I||' WHERE BARCODE ILIKE '''||KEYWORD||''' UNION ALL ';
	END LOOP;
		EX_SQL := EX_SQL || 'SELECT * FROM TRANSACTION_255 WHERE BARCODE ILIKE '''||KEYWORD||''' LIMIT '''||LIMIT2||''' OFFSET '''||OFFSET2||'''';
   ELSIF L = 19 THEN
	FOR I IN 0..254 LOOP
		EX_SQL := EX_SQL || 'SELECT * FROM TRANSACTION_'||I||' WHERE VIN ILIKE '''||KEYWORD||''' UNION ALL ';
	END LOOP;
		EX_SQL := EX_SQL || 'SELECT * FROM TRANSACTION_255 WHERE VIN ILIKE '''||KEYWORD||''' LIMIT '''||LIMIT2||''' OFFSET '''||OFFSET2||'''';
   ELSE
	FOR I IN 0..254 LOOP
		EX_SQL := EX_SQL || 'SELECT * FROM TRANSACTION_'||I||' WHERE BARCODE ILIKE '''||KEYWORD||''' OR PLATENUMBER ILIKE '''||KEYWORD||''' OR VIN ILIKE '''||KEYWORD||''' UNION ALL ';
	END LOOP;
		EX_SQL := EX_SQL || 'SELECT * FROM TRANSACTION_255 WHERE BARCODE ILIKE '''||KEYWORD||''' OR PLATENUMBER ILIKE '''||KEYWORD||''' OR VIN ILIKE '''||KEYWORD||''' LIMIT '''||LIMIT2||''' OFFSET '''||OFFSET2||'''';
...
SELECT * FROM CREATE_PAGINGCOUNT2(20, '%'||'11'||'%')$$

SELECT * FROM CREATE_SEARCH(20,0,'%'||'1'||'%')$$