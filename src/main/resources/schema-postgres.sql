...
BEGIN
   ...
   IF L = 9 OR L =10 THEN
	...
	END LOOP;
		EX_SQL := EX_SQL || 'SELECT * FROM TRANSACTION_255 WHERE PLATENUMBER ILIKE '''||KEYWORD||'''' || LIMIT_STR;
   ELSIF L = 15 THEN
	FOR I IN 0..254 LOOP
		EX_SQL := EX_SQL || 'SELECT * FROM TRANSACTION_'||I||' WHERE BARCODE ILIKE '''||KEYWORD||''' UNION ALL ';
	END LOOP;
		EX_SQL := EX_SQL || 'SELECT * FROM TRANSACTION_255 WHERE BARCODE ILIKE '''||KEYWORD||'''' || LIMIT_STR;
   ELSIF L = 19 THEN
	FOR I IN 0..254 LOOP
		EX_SQL := EX_SQL || 'SELECT * FROM TRANSACTION_'||I||' WHERE VIN ILIKE '''||KEYWORD||''' UNION ALL ';
	END LOOP;
		EX_SQL := EX_SQL || 'SELECT * FROM TRANSACTION_255 WHERE VIN ILIKE '''||KEYWORD||'''' || LIMIT_STR;
   ELSE
	FOR I IN 0..254 LOOP
	...
	END LOOP;
		EX_SQL := EX_SQL || 'SELECT * FROM TRANSACTION_255 WHERE BARCODE ILIKE '''||KEYWORD||''' OR PLATENUMBER ILIKE '''||KEYWORD||''' OR VIN ILIKE '''||KEYWORD||'''' || LIMIT_STR;
